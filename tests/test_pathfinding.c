 ...
 * Tests focus on pathfind_step_entity and pathfind_apply_direction using the
...
/* Build a Battlefield stub with manually generated straight-line waypoints
 * for SIDE_BOTTOM. This provides a predictable set of positions for testing
 * pathfind_step_entity without depending on bf_init's full logic. */
static Battlefield make_test_battlefield(void) {
    ...
    /* Generate simple straight-line waypoints for SIDE_BOTTOM (P1).
     * P1 territory: {0, 960, 1080, 960}
     * Forward direction: decreasing Y (toward top/enemy).
     * Spawn at ~80% from top of territory = y=1728
     * End at enemy side = y~192 (mirrored spawn) */
    float laneWidth = (float)BOARD_WIDTH / 3.0f;
    float spawnY = 960.0f + 960.0f * 0.8f;  /* 1728 */
    float endY = 192.0f;  /* opponent's spawn area */

    for (int lane = 0; lane < 3; lane++) {
        float centerX = (lane + 0.5f) * laneWidth;
        for (int wp = 0; wp < LANE_WAYPOINT_COUNT; wp++) {
            float t = (float)wp / (float)(LANE_WAYPOINT_COUNT - 1);
            float y = spawnY + (endY - spawnY) * t;
            bf.laneWaypoints[SIDE_BOTTOM][lane][wp].v = (Vector2){ centerX, y };
        }
