 ...
 *
 * Updated for Plan 11-03: pathfind_step_entity now takes const Battlefield *bf
 * instead of const Player *owner. Battlefield stub and bf_waypoint/bf_side_for_player
 * stubs are provided to keep tests self-contained.
 */
...
/*
 * Player stub: retained for lane_generate_waypoints (adapter function).
 */
...
/* Build a Battlefield stub populated with the same waypoints as the Player's
 * lane_generate_waypoints output (for SIDE_BOTTOM only). This lets the
 * pathfinding tests exercise the new Battlefield interface while keeping
 * the same expected positions. */
static Battlefield make_test_battlefield(const Player *p) {
    Battlefield bf;
    ...
    /* Copy Player waypoints into Battlefield canonical waypoints for SIDE_BOTTOM.
     * The Player P1 waypoints are already in the P1 coordinate space which,
     * for SIDE_BOTTOM, is identical to canonical space. */
    for (int lane = 0; lane < 3; lane++) {
        for (int wp = 0; wp < LANE_WAYPOINT_COUNT; wp++) {
            bf.laneWaypoints[SIDE_BOTTOM][lane][wp].v = p->laneWaypoints[lane][wp];
        }
    }
...
static void test_movement_step_advances_waypoint(void) {
    Player p = make_test_player();
    lane_generate_waypoints(&p);
    Battlefield bf = make_test_battlefield(&p);
